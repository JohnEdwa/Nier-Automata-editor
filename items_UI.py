...
class ItemsManagerUI(tk.Frame):
    ...
    def __init__(self, save_data, on_close=None):
        ...
        root.title("NieR;Automata Items Editor")
    ...
    def populate(self, parent):
        available_items = ("(Empty)",) + items.ItemsRecord.AVAILABLE_ITEMS
        # available_sizes = [i for i in range(31)]
        for row in range(self._items_manager.SAVE_DATA_ITEMS_COUNT):
            item_name = tk.StringVar()
            item_count = tk.StringVar(name="%u" % row)
            tk.Label(parent, text=str(row), width=3, borderwidth="1", relief="solid").grid(row=row, column=0)
            item_cb = tk.ttk.Combobox(parent, width=50, values=available_items, state="readonly", textvariable=item_name,
                                      name='combobox@%u' % row)
            item_cb.grid(row=row, column=1)
            count_sb = tk.Spinbox(parent, width=6, from_=0, to=99, textvariable=item_count,
                                  name='spinbox@%u' % row)
            count_sb.grid(row=row, column=2)
            id_tb = tk.Label(parent)
            id_tb.grid(row=row, column=3)

            current = self._items_manager.get_item_at(row)
            if current != items.ItemsRecord.EMPTY_RECORD:
                item_count.set(current.item_count)
                id_tb["text"] = '0x%04X' % current.item_id
                if -1 == current.name.find("Invalid"):
                    if 1 == current.uq:
                        count_sb.config(to=1)
                    #_available_items = (self.available_items[0], current.name)
                    #item_cb.config(values=available_items)
                    index = available_items.index(current.name)
                    item_cb.current(index)
    ...
    def on_item_cb_selected(self, event, args):
        ...
        if args[1].get() != "(Empty)":
            record = items.ItemsRecord.from_name(args[1].get())
            if 1 == record.uq:
                count_sb.config(to=1)
            else:
                count_sb.config(to=99)
    ...
    def on_count_sb_changed(self, *args):
        if 'w' == args[2]:
            ...
            count = count_sb.get()
            record = items.ItemsRecord.from_name(item_cb.get())
